...
for json_name in input_files:
    ...
    for j in range(len(data['objects'])):
        ...
        node_xmin.text = str(float(data['objects'][j]['bbox'][0])+1)  
        node_ymin = SubElement(node_bndbox, 'ymin')  
        node_ymin.text = str(float(data['objects'][j]['bbox'][1])+1)
        node_xmax = SubElement(node_bndbox, 'xmax')  
        node_xmax.text = str(float(data['objects'][j]['bbox'][0])+float(data['objects'][j]['bbox'][2]))
        node_ymax = SubElement(node_bndbox, 'ymax')  
        node_ymax.text = str(float(data['objects'][j]['bbox'][1])+float(data['objects'][j]['bbox'][3]))
        if ((float(node_xmax.text))>=float(node_width.text)):
        	node_xmax.text = str(float(node_width.text)-1)        
        if ((float(node_ymax.text))>=float(node_height.text)):
        	node_ymax.text = str(float(node_height.text)-1)        	              	              
