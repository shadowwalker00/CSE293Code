...
from model.bbox_transform import bbox_transform, bbox_overlaps
...
class Network(object):
  ...
  def bbox_iog(self, ground_truth, predicted):
  ...
  def create_repulsion_loss(self, alpha=0.5, betta=0.5,smooth_rep_gt=0.99, smooth_rep_box=0.01, objectness_to_filter=0.5):
    ...
    def _repulsion_impl(y_true, y_pred):
        ...
        ious = self.bbox_overlap_iou(y_pred, y_true)
        ...
        return tf.reduce_sum([            
            alpha * self.repulsion_term_gt(y_pred, ious_over_truth_boxes, smooth_rep_gt)
        ])
  ...
  def repulsion_term_gt(self, y_pred, ious_over_truth_boxes, smooth=0.99):
    ...
    ln_distances_for_iog = self.smooth_ln(self.bbox_iog(gt_boxes_with_max_ious, y_pred), smooth)
    result = tf.reduce_sum(ln_distances_for_iog) / tf.cast(tf.shape(y_pred)[1], dtype=tf.float32)
    return result 
  ...
  def _add_losses(self, sigma_rpn=3.0):
    with tf.variable_scope('LOSS_' + self._tag) as scope:
      ...
      bbox_pred = tf.expand_dims(bbox_pred, 0)
      bbox_targets = tf.expand_dims(bbox_targets, 0)

      repulsion_loss = self.create_repulsion_loss()
      repul_loss= repulsion_loss(bbox_pred, bbox_targets)
      ...
      self._losses['repulsion_loss'] = repul_loss

      loss = cross_entropy + loss_box + rpn_cross_entropy + rpn_loss_box + repul_loss
  ...
  def train_step_with_summary(self, sess, blobs, train_op):
    feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'],
                 self._gt_boxes: blobs['gt_boxes']}
    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, rep_loss, loss, summary, _ = sess.run([self._losses["rpn_cross_entropy"],
                                                                                 self._losses['rpn_loss_box'],
                                                                                 self._losses['cross_entropy'],
                                                                                 self._losses['loss_box'],
                                                                                 self._losses['repulsion_loss'],
                                                                                 self._losses['total_loss'],
                                                                                 self._summary_op,
                                                                                 train_op],
                                                                                feed_dict=feed_dict)
    return rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, rep_loss, loss, summary
